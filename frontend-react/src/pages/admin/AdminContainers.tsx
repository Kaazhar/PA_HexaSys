...
export default function AdminContainers() {
  ...
  return (
    <DashboardLayout sidebarItems={adminSidebar} title="Gestion des conteneurs">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              {['', 'pending', 'approved', 'rejected'].map((s) => (
                ...
                >
                  {s === '' ? 'Toutes' : containerRequestStatuses[s]?.label}
                </button>
              ))}
            </div>
          </div>
          ...
          {requestsLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                ...
                {requests.length === 0 && (
                  <EmptyState icon={<Package className="w-10 h-10" />} message="Aucune demande trouvée" />
                )}
              </div>
            </div>
          )}
