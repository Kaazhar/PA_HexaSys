...
export default function AdminArticles() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t('admin_articles.deleted')); queryClient.invalidateQueries({ queryKey: ['admin-articles'] }); },
    onError: (e: any) => toast.error(e?.response?.data?.error || t('common.error')),
  });
  ...
  const handleSave = () => {
    if (!form.title.trim() || !form.content.trim()) { toast.error(t('admin_articles.required')); return; }
    if (isNew) {
  ...
  return (
    <DashboardLayout sidebarItems={adminSidebar} title={t('admin_articles.title')}>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">{t('admin_articles.heading')}</h1>
          <button onClick={openCreate} className="btn-primary">
            {t('admin_articles.new_article')}
          </button>
        </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="table-header">{t('admin_articles.col_title')}</th>
                  <th className="table-header">{t('admin_articles.col_author')}</th>
                  <th className="table-header">{t('admin_articles.col_tags')}</th>
                  <th className="table-header">{t('admin_articles.col_status')}</th>
                  <th className="table-header">{t('admin_articles.col_views')}</th>
                  <th className="table-header">{t('admin_articles.col_date')}</th>
                  <th className="table-header">{t('admin_articles.col_actions')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                  ...
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">
                      <span className={`badge ${a.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                        {a.status === 'published' ? t('admin_articles.published') : t('admin_articles.draft')}
                      </span>
                    </td>
                    ...
                    <td className="table-cell">
                      <div className="flex gap-1.5">
                        ...
                        <button onClick={() => { if (confirm(t('admin_articles.confirm_delete', { title: a.title }))) deleteMutation.mutate(a.id); }} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                ...
                {articles.length === 0 && (
                  <tr><td colSpan={7} className="py-12 text-center text-gray-400">{t('admin_articles.no_articles')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-2xl w-full shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold">{isNew ? t('admin_articles.modal_create') : t('admin_articles.modal_edit')}</h3>
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_articles.label_title')}</label>
                <input className="input w-full" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_articles.label_content')}</label>
                <textarea className="input w-full h-48 resize-none" value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_articles.label_tags')}</label>
                  <input className="input w-full" value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} placeholder={t('admin_articles.tags_ph')} />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_articles.label_status')}</label>
                  <select className="input w-full" value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
                    <option value="published">{t('admin_articles.published')}</option>
                    <option value="draft">{t('admin_articles.draft')}</option>
                  </select>
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-5">
              ...
              <button onClick={handleSave} disabled={createMutation.isPending || updateMutation.isPending} className="btn-primary flex items-center gap-2">
                <Check className="w-4 h-4" /> {isNew ? t('common.create') : t('admin_articles.save')}
              </button>
            </div>
          </div>
        </div>
      ...
      {viewArticle && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-2xl w-full shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-sm text-gray-500 mb-1">{t('admin_articles.by')} {viewArticle.author ? `${viewArticle.author.firstname} ${viewArticle.author.lastname}` : `#${viewArticle.author_id}`} · {format(new Date(viewArticle.created_at), 'dd MMM yyyy', { locale: fr })}</p>
            ...
            <div className="flex justify-end gap-3 mt-5">
              <button onClick={() => { openEdit(viewArticle); setViewArticle(null); }} className="btn-secondary flex items-center gap-2"><Pencil className="w-4 h-4" /> {t('admin_articles.modify')}</button>
              <button onClick={() => setViewArticle(null)} className="btn-primary">{t('admin_articles.close')}</button>
            </div>
          </div>
        </div>
      )}
