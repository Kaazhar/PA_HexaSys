...
export default function AdminSubscriptionPlans() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t('admin_plans.deleted')); queryClient.invalidateQueries({ queryKey: ['admin-plans'] }); },
    onError: (e: any) => toast.error(e?.response?.data?.error || t('common.error')),
  });
  ...
  return (
    <DashboardLayout sidebarItems={adminSidebar} title={t('admin_plans.title')}>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">{t('admin_plans.title')}</h1>
          <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-left text-xs text-gray-500 uppercase">
                  <th className="pb-2 pr-4">{t('admin_plans.col_name')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_slug')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_price')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_listings')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_duration')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_active')}</th>
                  <th className="pb-2 pr-4">{t('admin_plans.col_order')}</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3 pr-4">{plan.price === 0 ? t('admin_plans.free') : t('admin_plans.price_month', { price: plan.price })}</td>
                    <td className="py-3 pr-4">+{plan.max_listings_bonus}</td>
                    <td className="py-3 pr-4">{plan.duration_days}j</td>
                    <td className="py-3 pr-4">
                      {plan.is_active
                        ? <span className="text-green-600 font-medium">{t('admin_plans.yes')}</span>
                        : <span className="text-gray-400">{t('admin_plans.no')}</span>}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold">{isNew ? t('admin_plans.modal_create') : t('admin_plans.modal_edit')}</h3>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_name')}</label>
                  <input className="input w-full" value={editing.name ?? ''} onChange={e => setEditing(p => ({ ...p!, name: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_slug')}</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_price')}</label>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_listings')}</label>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_duration')}</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-medium text-gray-600">{t('admin_plans.label_features')}</label>
                  <button type="button" onClick={addFeature} className="flex items-center gap-1 text-xs text-primary-600 hover:text-primary-700 font-medium">
                    <Plus className="w-3.5 h-3.5" /> {t('admin_plans.add_feature')}
                  </button>
                </div>
                <div className="space-y-2">
                  {featureItems.length === 0 && (
                    <p className="text-xs text-gray-400 italic py-2">{t('admin_plans.no_features')}</p>
                  )}
                  {featureItems.map((f, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <input
                        ...
                        placeholder={t('admin_plans.feature_ph', { n: i + 1 })}
                      />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">{t('admin_plans.label_order')}</label>
                ...
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-700">{t('admin_plans.label_active')}</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-5">
              ...
              <button onClick={handleSave} disabled={createMutation.isPending || updateMutation.isPending} className="btn-primary flex items-center gap-2">
                ...
                {isNew ? t('common.create') : t('admin_plans.save')}
              </button>
            </div>
          </div>
        </div>
