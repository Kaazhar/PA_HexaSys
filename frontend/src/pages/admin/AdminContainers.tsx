...
import { LayoutDashboard, Users, Tag, BookOpen, Package, DollarSign, Settings, Plus, CheckCircle, XCircle, MapPin, Trash2 } from 'lucide-react';
...
export default function AdminContainers() {
  ...
  const [clearTarget, setClearTarget] = useState<{ id: number; name: string } | null>(null);
  ...
  const clearSlotsMutation = useMutation({
    mutationFn: (id: number) => containerService.clearSlots(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['containers'] });
      setClearTarget(null);
      toast.success('Stock vidé');
    },
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} title="Gestion des conteneurs">
      <div className="space-y-8">
                ...
                return (
                  <div key={container.id} className="card">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setClearTarget({ id: container.id, name: container.name })}
        ...
        {/* Clear Slots Modal */}
        <Modal isOpen={!!clearTarget} onClose={() => setClearTarget(null)} title="Vider le stock" size="sm">
          <p className="text-sm text-gray-600 mb-5">
            Remettre tous les slots de <strong>{clearTarget?.name}</strong> à "libre" et remettre le compteur à 0 ?
            Cette action est irréversible.
          </p>
          <div className="flex gap-3">
            <button type="button" onClick={() => setClearTarget(null)} className="btn-secondary flex-1">Annuler</button>
            <button
              type="button"
              disabled={clearSlotsMutation.isPending}
              onClick={() => clearTarget && clearSlotsMutation.mutate(clearTarget.id)}
              className="btn-danger flex-1"
            >
              {clearSlotsMutation.isPending ? 'Suppression...' : 'Vider'}
            </button>
          </div>
        </Modal>
